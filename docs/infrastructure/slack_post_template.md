...
- TATCHU3 (172.25.64.3): DataJoint DB host, blob host, ingest share host, CPU worker host
- ibehaveGPU1 (172.25.70.3): GPU workers + model training/evaluation host
- LOKI (172.26.65.8): backup host (rsync pull + borg compression)
...
[adamacs_ingest GUI + batch scripts]
        ...
        +--> [DataJoint DB]
        |          |
        |          +--> [TATCHU CPU workers] --> [/datajoint-db/blobs/...]
        |          |
        |          +--> [ibehaveGPU1 GPU workers] --> [/datajoint-db/blobs/...]
        |
        +--> [Task tables]

[Data + blobs + DB dumps] --> [LOKI backup pulls] --> [borg zstd,22 archives]

Closed-loop
ibehaveGPU1 model training/evaluation can feed stimulation candidates back to bench2p1 (MEI restimulation), and those new recordings re-enter ingest.
...
For Shan specifically: please check if mocap.MotionCapture for your scan is already populated now.
```
